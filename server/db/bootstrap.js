...
async function createBarbers() {
    const barbers = [new Barber({ name: "Barber A" }), new Barber({ name: "Barber B" })];
...
async function createSchedules(timePeriod = 7) {
    ...
    appointmentsA.forEach(appointment => appointment.save())
    appointmentsB.forEach(appointment => appointment.save())
